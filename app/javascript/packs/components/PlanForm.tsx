...
const PlanForm = (props: IPlanFormProps) => {
  ...
  const [seed, setSeed] = useState(props.initialCover ? parseInt(props.initialCover.split('/seed/')[1]) : Math.floor(Math.random() * 100000));
  ...
  const regeneratePhoto = () => {
    ...
    setLoading(true);
    if (props.onCoverChange) {
      props.onCoverChange(`https://picsum.photos/seed/${seed}`);
    }
  };
